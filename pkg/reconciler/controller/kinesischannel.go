...
import (
	...
	messaginginformers "github.com/triggermesh/aws-kinesis-channel/pkg/client/informers/externalversions/messaging/v1alpha1"
...
func (r *Reconciler) reconcile(ctx context.Context, kc *v1alpha1.KinesisChannel) error {
	...
	if kc.DeletionTimestamp != nil {
		if kc.Status.GetCondition(v1alpha1.KinesisChannelConditionStreamReady).IsTrue() {
			...
			kclient, err := r.kinesisClient(kc.Name, kc.Spec.AccountRegion, creds)
			if err != nil {
				return err
			}
	...
	if kc.Status.GetCondition(v1alpha1.KinesisChannelConditionStreamReady).IsUnknown() ||
		...
		default:
			for {
				res, err := kinesisutil.Describe(ctx, kclient, kc.Name)
				if err != nil {
					return err
				}
				if *res.StreamDescription.StreamStatus == kinesis.StreamStatusActive && len(res.StreamDescription.Shards) > 0 {
					break
				}
