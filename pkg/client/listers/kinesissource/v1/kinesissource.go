...
type KinesisSourceLister interface {
	...
	List(selector labels.Selector) (ret []*v1.KinesisSource, err error)
...
// List lists all KinesisSources in the indexer.
func (s *kinesisSourceLister) List(selector labels.Selector) (ret []*v1.KinesisSource, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.KinesisSource))
	})
	return ret, err
}
...
type KinesisSourceNamespaceLister interface {
	...
	Get(name string) (*v1.KinesisSource, error)
...
// List lists all KinesisSources in the indexer for a given namespace.
func (s kinesisSourceNamespaceLister) List(selector labels.Selector) (ret []*v1.KinesisSource, err error) {
...
// Get retrieves the KinesisSource from the indexer for a given namespace and name.
func (s kinesisSourceNamespaceLister) Get(name string) (*v1.KinesisSource, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("kinesissource"), name)
	}
	return obj.(*v1.KinesisSource), nil
}
