...
import (
	...
	samplev1alpha1 "github.com/triggermesh/aws-kinesis-provisioner/pkg/apis/kinesiscontroller/v1alpha1"
	clientset "github.com/triggermesh/aws-kinesis-provisioner/pkg/generated/clientset/versioned"
	samplescheme "github.com/triggermesh/aws-kinesis-provisioner/pkg/generated/clientset/versioned/scheme"
	informers "github.com/triggermesh/aws-kinesis-provisioner/pkg/generated/informers/externalversions/kinesiscontroller/v1alpha1"
	listers "github.com/triggermesh/aws-kinesis-provisioner/pkg/generated/listers/kinesiscontroller/v1alpha1"
)

const controllerAgentName = "kinesiscontroller"
...
func NewController(
	...
	channelInformer informers.ChannelInformer) *Controller {
...
func (c *Controller) Run(threadiness int, stopCh <-chan struct{}) error {
	...
	klog.Info("Starting Channel controller")

	// Wait for the caches to be synced before starting workers
	klog.Info("Waiting for informer caches to sync")
	if ok := cache.WaitForCacheSync(stopCh, c.deploymentsSynced, c.channelsSynced); !ok {
...
func (c *Controller) syncHandler(key string) error {
	...
	if err != nil {
		...
		if errors.IsNotFound(err) {
			utilruntime.HandleError(fmt.Errorf("channel '%s' in work queue no longer exists", key))
			return nil
		}
	...
	deploymentName := channel.Spec.DeploymentName
	if deploymentName == "" {
	...
	// Get the deployment with the name specified in Channel.spec
	deployment, err := c.deploymentsLister.Deployments(channel.Namespace).Get(deploymentName)
	// If the resource doesn't exist, we'll create it
	if errors.IsNotFound(err) {
		deployment, err = c.kubeclientset.AppsV1().Deployments(channel.Namespace).Create(newDeployment(channel))
	}
	...
	// If the Deployment is not controlled by this Channel resource, we should log
	// a warning to the event recorder and ret
	if !metav1.IsControlledBy(deployment, channel) {
		msg := fmt.Sprintf(MessageResourceExists, deployment.Name)
		c.recorder.Event(channel, corev1.EventTypeWarning, ErrResourceExists, msg)
		return fmt.Errorf(msg)
	}

	// If this number of the replicas on the Channel resource is specified, and the
	// number does not equal the current desired replicas on the Deployment, we
	// should update the Deployment resource.
	if channel.Spec.Replicas != nil && *channel.Spec.Replicas != *deployment.Spec.Replicas {
		klog.V(4).Infof("Channel %s replicas: %d, deployment replicas: %d", name, *channel.Spec.Replicas, *deployment.Spec.Replicas)
		deployment, err = c.kubeclientset.AppsV1().Deployments(channel.Namespace).Update(newDeployment(channel))
	}
	...
	err = c.updateChannelStatus(channel, deployment)
	if err != nil {
		return err
	}
...
func (c *Controller) updateChannelStatus(channel *samplev1alpha1.Channel, deployment *appsv1.Deployment) error {
	// NEVER modify objects from the store. It's a read-only, local cache.
	// You can use DeepCopy() to make a deep copy of original object and modify this copy
	// Or create a copy manually for better performance
	channelCopy := channel.DeepCopy()
	channelCopy.Status.AvailableReplicas = deployment.Status.AvailableReplicas
...
func (c *Controller) handleObject(obj interface{}) {
	...
	if ownerRef := metav1.GetControllerOf(object); ownerRef != nil {
		...
		if err != nil {
			klog.V(4).Infof("ignoring orphaned object '%s' of channel '%s'", object.GetSelfLink(), ownerRef.Name)
			return
		}
...
// newDeployment creates a new Deployment for a Channel resource. It also sets
// the appropriate OwnerReferences on the resource so handleObject can discover
// the Channel resource that 'owns' it.
func newDeployment(channel *samplev1alpha1.Channel) *appsv1.Deployment {
	...
	return &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
			Name:      channel.Spec.DeploymentName,
			Namespace: channel.Namespace,
			OwnerReferences: []metav1.OwnerReference{
				*metav1.NewControllerRef(channel, samplev1alpha1.SchemeGroupVersion.WithKind("Channel")),
			},
		},
