...
type fixture struct {
	...
	fooLister        []*samplecontroller.Channel
...
func newChannel(name string, replicas *int32) *samplecontroller.Channel {
	return &samplecontroller.Channel{
		TypeMeta: metav1.TypeMeta{APIVersion: samplecontroller.SchemeGroupVersion.String()},
		ObjectMeta: metav1.ObjectMeta{
			Name:      name,
			Namespace: metav1.NamespaceDefault,
		},
		Spec: samplecontroller.ChannelSpec{
...
func (f *fixture) newController() (*Controller, informers.SharedInformerFactory, kubeinformers.SharedInformerFactory) {
	...
	c := NewController(f.kubeclient, f.client,
		k8sI.Apps().V1().Deployments(), i.Samplecontroller().V1alpha1().Channels())
	...
	for _, f := range f.fooLister {
		i.Samplecontroller().V1alpha1().Channels().Informer().GetIndexer().Add(f)
	}
...
func (f *fixture) expectUpdateChannelStatusAction(foo *samplecontroller.Channel) {
...
func getKey(foo *samplecontroller.Channel, t *testing.T) string {
...
func TestCreatesDeployment(t *testing.T) {
	f := newFixture(t)
	foo := newChannel("test", int32Ptr(1))
	...
	f.expectUpdateChannelStatusAction(foo)
