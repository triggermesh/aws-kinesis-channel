...
func getKubernetesClient() (kubernetes.Interface, kinesissourceclientset.Interface) {
	...
	kinesissourceClient, err := kinesissourceclientset.NewForConfig(config)
	if err != nil {
		log.Fatalf("getClusterConfig: %v", err)
	}

	log.Info("Successfully constructed k8s client")
	return client, kinesissourceClient
}

// main code path
func main() {
	// get the Kubernetes client for connectivity
	client, kinesissourceClient := getKubernetesClient()
	...
	informer := kinesissourceinformer_v1.NewKinesisSourceInformer(
		kinesissourceClient,
	...
	informer.AddEventHandler(cache.ResourceEventHandlerFuncs{
		AddFunc: func(obj interface{}) {
			...
			log.Infof("Add kinesissource: %s", key)
			if err == nil {
		...
		UpdateFunc: func(oldObj, newObj interface{}) {
			...
			log.Infof("Update kinesissource: %s", key)
			if err == nil {
				queue.Add(key)
			}
		},
		DeleteFunc: func(obj interface{}) {
			...
			log.Infof("Delete kinesissource: %s", key)
			if err == nil {
				queue.Add(key)
			}
		},
